...
export function getDefaultFormState(
  ...
  rootSchema = {},
  includeUndefinedValues = false,
  onRetrieveSchema = null
) {
  if (!isObject(_schema)) {
    throw new Error("Invalid schema: " + _schema);
  }
  const schema = retrieveSchema(_schema, rootSchema, formData, onRetrieveSchema);
...
export function findSchemaDefinition($ref, rootSchema = {}, onRetrieveSchema = null) {
  const origRef = $ref;
  let current;
  if ($ref.startsWith("@")) {
 ...
 */
export function resolveSchema(schema, rootSchema = {}, formData = {}, onRetrieveSchema = null) {
  if (schema.hasOwnProperty("$ref")) {
    return resolveReference(schema, rootSchema, formData, onRetrieveSchema);
  } else if (schema.hasOwnProperty("dependencies")) {
    const resolvedSchema = resolveDependencies(schema, rootSchema, formData, onRetrieveSchema);
    return retrieveSchema(resolvedSchema, rootSchema, formData, onRetrieveSchema);
  } else if (schema.hasOwnProperty("allOf")) {
    return {
      ...schema,
      allOf: schema.allOf.map(allOfSubschema =>
        retrieveSchema(allOfSubschema, rootSchema, formData, onRetrieveSchema)
      ),
    };
  } else {
...
function resolveReference(schema, rootSchema, formData, onRetrieveSchema = null) {
  // Retrieve the referenced schema definition.
  const $refSchema = findSchemaDefinition(schema.$ref, rootSchema, onRetrieveSchema);
...
export function retrieveSchema(schema, rootSchema = {}, formData = {}, onRetrieveSchema = null) {
  if (!isObject(schema)) {
    return {};
  }
  let resolvedSchema = resolveSchema(schema, rootSchema, formData, onRetrieveSchema);
...
function resolveDependencies(schema, rootSchema, formData, onRetrieveSchema = null) {
...
function withExactlyOneSubschema(
  ...
  return mergeSchemas(
    schema,
    retrieveSchema(dependentSchema, rootSchema, formData, onRetrieveSchema)
  );
}
